  ...
  <DefaultLayout>
    <v-container class="mt-5">
            ...
            <v-col cols="12">
              <v-row>
                <v-col cols="2" v-for="n in 75" :key="n" class="d-flex justify-center">
                  <v-sheet class="number-cell" :class="{ 'selected-number': game?.drawnNumbers.includes(n) }">
                    {{ n }}
                  </v-sheet>
                </v-col>
              </v-row>
      ...
      <v-row class="d-flex justify-center">
        <v-col cols="12" md="8">
          <v-row>
            <v-col cols="12" md="6" v-for="(player, index) in game?.players" :key="index" class="mb-4">
              <CartelaBingo :player-name="player.name" :ticket-numbers="player.ticketNumbers"
